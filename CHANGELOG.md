...
### Added
- **Cloud agent environments (new)**: `cloud/install-gstack.sh` — installs [gstack](https://github.com/garrytan/gstack) on a headless cloud box and registers its skills for the claude, codex, and cursor hosts. Exists because nothing user-global crosses into a Conductor cloud workspace: `~/.claude` is Mac-local, files-to-copy runs for local Mac workspaces only, and `scripts.setup` in `.conductor/settings.toml` does not run in cloud — the Cloud Computer "Install software" script is the only persistent hook. Installs bun (not on the cloud image; gstack's `./setup` hard-fails without it) and persists its PATH to both `~/.bash_profile` and `~/.profile`. Uses `--prefix` so skills land as `/gstack-ship` / `/gstack-qa` instead of bare names that collide with harness- and repo-provided skills, and `--no-team` so the snapshot doesn't carry gstack's SessionStart auto-update hook into every workspace. Pins via `GSTACK_REF`, hosts via `GSTACK_HOSTS`. Probes headless Chromium rather than assuming it (a missing browser silently disables `/gstack-browse`); points at `home-assistant-config/.claude/scripts/install-gstack.sh` for the proxied-CDN fallback. Idempotent — verified on a Conductor cloud sandbox (Amazon Linux 2023): ~30 s and 1.4 GB cold, ~12 s as a re-run, 54 skills registered.
